...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return guarded(async (session) => {
    ...
    if (logo instanceof File) {
      kit.logoRef = await putFile(ws, `clients/${id}/brand/logo-${logo.name}`, Buffer.from(await logo.arrayBuffer()), logo.type);
    }
    ...
    if (artwork.length) {
      ...
      for (const f of artwork.slice(0, 6 - refs.length)) {
        refs.push(await putFile(ws, `clients/${id}/brand/art-${refs.length}-${f.name}`, Buffer.from(await f.arrayBuffer()), f.type));
      }
      kit.artworkRefs = refs;
    }
    const updated = await updateClient(ws, id, { brandKit: kit });
