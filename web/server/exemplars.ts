...
const path = (clientId: string) => `users/${uid()}/clients/${clientId}/exemplars`;
...
export async function recordExemplar(clientId: string, asset: AssetDoc, campaignId: string, brief: any, kind: 'approved' | 'rejected') {
  const doc = {
    exemplarId: newId(),
  ...
  if (storeEnabled) await fsdb().doc(`${path(clientId)}/${doc.exemplarId}`).set(doc);
  else mem.set(clientId, [...(mem.get(clientId) || []), doc]);
  return doc;
}

export async function listExemplars(clientId: string) {
  if (!storeEnabled) return mem.get(clientId) || [];
  return (await fsdb().collection(path(clientId)).get()).docs.map((d) => d.data());
}

/** After results arrive, attach performance to the exemplars whose asset matched. */
export async function attachPerformance(clientId: string, rows: any[], metric = 'conversions') {
  const all = await listExemplars(clientId);
  ...
  for (const e of all) {
    ...
    if (storeEnabled) await fsdb().doc(`${path(clientId)}/${e.exemplarId}`).set(updated);
