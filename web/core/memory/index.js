 ...
 */
async function exemplars({ ws, clientId, agent, channel, objective, limit = 6 }) {
  if (!clientId) return [];
  const where = [['kind', '==', 'approved']];
  if (channel) where.push(['channel', '==', channel]);
  else if (agent) where.push(['agent', '==', agent]);
  let rows = await read(ws, clientId, 'exemplars', { where, order: 'approvedAt', limit: limit * 3 });
  if (!rows.length) {
    rows = fallback('exemplars', ws, clientId).filter((e) => e.kind === 'approved' && (!channel || e.channel === channel) && (channel || !agent || e.agent === agent));
  }
...
async function learnings({ ws, clientId }) {
  if (!clientId) return [];
  const rows = await read(ws, clientId, 'learnings', { where: [['status', '==', 'approved']], order: 'approvedAt', limit: 12 });
  if (rows.length) return rows;
  return fallback('learnings', ws, clientId).filter((l) => l.status === 'approved').slice(0, 12);
}

/** Corrections the editor has given for this client, so they are not repeated. */
async function corrections({ ws, clientId, agent }) {
  const where = [['status', '==', 'confirmed']];
  if (agent) where.push(['agent', '==', agent]);
  return read(ws, clientId, 'corrections', { where, order: 'createdAt', limit: 10 });
}
 ...
 * become a violation.
 */
async function approvedClaims({ ws, clientId }) {
  if (!clientId) return null;
  const now = new Date().toISOString();
  let rows = await read(ws, clientId, 'claims', { where: [['status', '==', 'approved']], limit: 100 });
  if (!rows.length) rows = fallback('claims', ws, clientId).filter((c) => c.status === 'approved');
