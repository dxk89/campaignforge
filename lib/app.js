...
app.post('/api/pass/audience', wrap(async (req, res) => {
  const brief = requireBrief(req, res); if (!brief) return;
  res.json(await chain.passAudience({ brief, context: req.body.context, webResearch: req.body.webResearch }));
}));

app.post('/api/pass/strategy', wrap(async (req, res) => {
  const brief = requireBrief(req, res); if (!brief) return;
  res.json(await chain.passStrategy({ brief, context: req.body.context, audience: req.body.audience }));
}));

app.post('/api/pass/assets', wrap(async (req, res) => {
  const brief = requireBrief(req, res); if (!brief) return;
  if (!req.body.strategy) return res.status(400).json({ error: 'strategy is required' });
  res.json(await chain.passAssets({ brief, strategy: req.body.strategy, context: req.body.context, audience: req.body.audience }));
}));
...
app.post('/api/pass/social', wrap(async (req, res) => {
  ...
  res.json(await chain.passSocial({ brief, strategy: req.body.strategy, assets: req.body.assets, context: req.body.context, audience: req.body.audience, brandKit: req.body.brandKit }));
}));

app.post('/api/pass/activation', wrap(async (req, res) => {
  const brief = requireBrief(req, res); if (!brief) return;
  if (!req.body.strategy || !req.body.assets) return res.status(400).json({ error: 'strategy and assets are required' });
  res.json(await chain.passActivation({ brief, strategy: req.body.strategy, assets: req.body.assets, context: req.body.context, audience: req.body.audience, landingUrl: req.body.landingUrl }));
}));
