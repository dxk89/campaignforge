...
async function passActivation({ brief, strategy: strat, assets, context, landingUrl }) {
  const ctx = context || research.emptyContext();
  const a = await callJson({
    system: activation.systemPrompt(),
    user: activation.userPrompt(brief, strat, assets, research.contextForPrompt(ctx)),
    maxTokens: 5000,
    label: 'activation',
  });
  return {
...
async function runChain(brief) {
  ...
  const act = await passActivation({ brief, strategy: s.strategy, assets: a.assets, context: r.context, landingUrl: brief.landingUrl });
...
module.exports = { validateBrief, passResearch, passStrategy, passAssets, passActivation, passLocalise, runChain, summarise, parseBrief };
